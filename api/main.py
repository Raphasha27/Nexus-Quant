...
def generate_ohlcv(base: float = 150.0, n: int = 50):
    ...
    for i in range(n):
        ...
        data.append({"date": date, "open": open_p, "high": high_p, "low": low_p, "close": close_p, "volume": volume})
        price = close_p
    return data
...
@app.get("/")
def root():
    return {"status": "ONLINE", "platform": "Nexus-Quant", "version": "2.0.0", "modes": ["signal", "anomaly", "portfolio"]}
...
def generate_signal(req: SignalRequest):
    ...
    signal = "BUY" if sma_5 > sma_20 and rsi < 70 else "SELL" if sma_5 < sma_20 and rsi > 30 else "HOLD"
    return {
        "ticker": req.ticker,
...
def detect_anomaly(req: AnomalyRequest):
    ...
    anomalies = [{"date": c["date"], "volume": c["volume"], "z_score": round((c["volume"] - avg_vol) / (avg_vol * 0.3), 2)}
                 for c in ohlcv if abs(c["volume"] - avg_vol) > avg_vol * 0.5]
    return {
        "ticker": req.ticker,
...
def optimize_portfolio(req: PortfolioRequest):
    ...
    allocations = {t: {"weight": w, "capital_usd": round(w * req.capital, 2)} for t, w in zip(req.tickers, weights)}
    return {
